...
export default function EditPaymentCardForm({closaModal, card}: {closaModal: ()=>void, card: IPaymentModel}) {
    const {t} = useT('payment_form');
    const [cardHolder, setCardHolder] = useState(card.ownerName);
    const [cardNumber, setCardNumber] = useState(card.number);
    const [month, setMonth] = useState(card.date.split('/')[0]);
    const [year, setYear] = useState(card.date.split('/')[1]);
    const [cvc, setCvc] = useState(card.cvv);
    const [errors, setErrors] = useState<FormErrors>({});
    const [submitted, setSubmitted] = useState(false);
    const [isFlipped, setIsFlipped] = useState(false);
    const [editCard] = paymentApi.useEditCardMutation()
