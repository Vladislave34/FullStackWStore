...
const SideBarLabel: FC<NavBarLabelProps> = ({href, children, subItems}) => {
    ...
    const isActive = (href : string, path : string) => {
        if(href.includes("page")){
            return path === href.slice(0, href.length-7);
        }
        return path === href;
    }
                    ...
                    <div className="flex flex-col gap-1 pl-4">
                        {subItems.map((item) => (
                            <Link
                                key={item.href}
                                href={item.href}
                                className={`${baseClass} justify-start  hover:translate-x-2 transition-all w-full text-xs ${isActive(item.href, path) ? activeClass+"translate-x-2" : ""}`}
                            >
                                {item.label}
                            </Link>
                        ))}
