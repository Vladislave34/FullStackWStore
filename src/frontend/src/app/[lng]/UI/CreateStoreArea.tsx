...
import CreateStoreForm from "@/app/[lng]/UI/forms/CreateStoreForm";
...
const CreateStoreArea  = () => {
    const modal = useModal();
    const {t} = useT('create_store')
    return (
        <>
        <div className=" flex flex-col gap-4 p-8 w-1/2 border-x-2 border-[var(--border)] ">
            ...
            <div
             ...
             ">
                <div>{t('title_1')}</div>
                <div>{t('title_2')}</div>
                <div className="text-[var(--accent-soft)]">{t('title_3')}</div>
            </div>
            ...
            <Modal title={"Create store"} isOpen={modal.isOpen} closeModal={modal.closeModal}>
                <CreateStoreForm closeModal={modal.closeModal}   />
            </Modal>
        </>
    );
};
